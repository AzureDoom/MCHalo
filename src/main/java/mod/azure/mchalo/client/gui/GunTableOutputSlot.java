...
public class GunTableOutputSlot extends Slot {
	...
	protected void checkTakeAchievements(ItemStack stack) {
		stack.onCraftedBy(this.player.getCommandSenderWorld(), this.player, this.amount);
		this.amount = 0;
	}

	public void onTake(Player player, ItemStack stack) {
		this.checkTakeAchievements(stack);
		var optionalGunTableRecipe = player.getCommandSenderWorld().getRecipeManager().getRecipeFor(Type.INSTANCE, gunTableInventory,
