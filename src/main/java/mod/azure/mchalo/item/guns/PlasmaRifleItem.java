...
public class PlasmaRifleItem extends HaloGunBase {
	...
	public PlasmaRifleItem() {
		super(new Item.Settings().group(MCHaloMod.HALOTAB).maxCount(1).maxDamage(config.plasmarifle_max_ammo + 1));
	}
			...
			if (stack.getDamage() < (stack.getMaxDamage() - 1)
				...
				if (!worldIn.isClient) {
					PlasmaEntity abstractarrowentity = createPlamsa(worldIn, stack, playerentity,
							config.plasmarifle_bullet_damage);
	...
	public void appendTooltip(ItemStack stack, World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(new TranslatableText("Damage: "
				+ (j > 0 ? (config.plasmarifle_bullet_damage + (j * 1.5F + 0.5F)) : config.plasmarifle_bullet_damage))
