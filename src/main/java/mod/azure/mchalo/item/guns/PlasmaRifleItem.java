...
public class PlasmaRifleItem extends HaloGunBase {
	...
	public PlasmaRifleItem() {
		super(new Item.Settings().maxCount(1).maxDamage(HaloConfig.plasmarifle_max_ammo + 1));
			...
			if (stack.getDamage() < (stack.getMaxDamage() - 1)
				...
				if (!worldIn.isClient) {
					PlasmaEntity abstractarrowentity = createPlamsa(worldIn, stack, playerentity,
							HaloConfig.plasmarifle_bullet_damage);
					...
					worldIn.playSound((PlayerEntity) null, playerentity.getX(), playerentity.getY(),
							playerentity.getZ(), HaloSounds.PLASMARIFLE, SoundCategory.PLAYERS, 0.25F, 1.3F);
	...
	public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
		if (world.isClient) {
			if (((PlayerEntity) entity).getMainHandStack().getItem() instanceof PlasmaRifleItem) {
	...
	public void reload(PlayerEntity user, Hand hand) {
		if (user.getStackInHand(hand).getItem() instanceof PlasmaRifleItem) {
			while (!user.isCreative() && user.getStackInHand(hand).getDamage() != 0
				...
				user.getStackInHand(hand).damage(-HaloConfig.plasmarifle_mag_size, user,
	...
	public void appendTooltip(ItemStack stack, World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("Damage: " + (j > 0 ? (HaloConfig.plasmarifle_bullet_damage + (j * 1.5F + 0.5F))
				: HaloConfig.plasmarifle_bullet_damage)).formatted(Formatting.ITALIC));
	}
	...
	public void createRenderer(Consumer<Object> consumer) {
		consumer.accept(new RenderProvider() {
			private final PlasmaRifleRender renderer = new PlasmaRifleRender();
