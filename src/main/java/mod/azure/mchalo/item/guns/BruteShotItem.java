...
public class BruteShotItem extends HaloGunBase {
	...
	public BruteShotItem() {
		super(new Item.Properties().stacksTo(1).durability(HaloConfig.bruteshot_max_ammo + 1));
	...
	public void reload(Player user, InteractionHand hand) {
		if (user.getItemInHand(hand).getItem() instanceof BruteShotItem) {
			while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0
				...
				user.getItemInHand(hand).hurtAndBreak(-HaloConfig.bruteshot_mag_size, user,
	...
	public void appendHoverText(ItemStack stack, Level world, List<Component> tooltip, TooltipFlag context) {
		...
		tooltip.add(
				Component.translatable("Damage: " + (j > 0 ? (HaloConfig.bruteshot_bullet_damage + (j * 1.5F + 0.5F))
						: HaloConfig.bruteshot_bullet_damage)).withStyle(ChatFormatting.ITALIC));
	}
