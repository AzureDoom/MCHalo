...
public class NeedlerItem extends HaloGunBase {
	...
	public NeedlerItem() {
		super(new Item.Settings().maxCount(1).maxDamage(HaloConfig.needler_max_ammo + 1));
			...
			if (stack.getDamage() < (stack.getMaxDamage() - 1)
				...
				if (!worldIn.isClient) {
					NeedleEntity abstractarrowentity = createNeedle(worldIn, stack, playerentity,
							HaloConfig.needler_bullet_damage);
					...
					worldIn.playSound((PlayerEntity) null, playerentity.getX(), playerentity.getY(),
							playerentity.getZ(), HaloSounds.NEEDLER, SoundCategory.PLAYERS, 0.5F,
	...
	public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
		if (world.isClient) {
			if (((PlayerEntity) entity).getMainHandStack().getItem() instanceof NeedlerItem) {
	...
	public void reload(PlayerEntity user, Hand hand) {
		if (user.getStackInHand(hand).getItem() instanceof NeedlerItem) {
			while (!user.isCreative() && user.getStackInHand(hand).getDamage() != 0
					&& user.getInventory().count(HaloItems.NEEDLES) > 0) {
				removeAmmo(HaloItems.NEEDLES, user);
				user.getStackInHand(hand).damage(-HaloConfig.needler_mag_size, user,
	...
	public void createRenderer(Consumer<Object> consumer) {
		consumer.accept(new RenderProvider() {
			private final NeedlerRender renderer = new NeedlerRender();
