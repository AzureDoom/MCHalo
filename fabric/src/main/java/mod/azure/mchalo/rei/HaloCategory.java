...
public class HaloCategory implements DisplayCategory<HaloDisplay> {
	...
	public static final EntryStack<ItemStack> ICON = EntryStacks.of(FabricLibMod.GUN_TABLE);
	...
	@Override
	public CategoryIdentifier<? extends HaloDisplay> getCategoryIdentifier() {
		return ReiPlugin.CRAFTING;
	}

	@Override
	public List<Widget> setupDisplay(HaloDisplay display, Rectangle bounds) {
		...
		if (display.count.get(0).intValue() > 0) {
			...
			widgets.add(Widgets
					.createLabel(new Point(display.count.get(0).intValue() < 10 ? startPoint.x + 17 : startPoint.x + 13,
							startPoint.y + 1), Component.translatable(display.count.get(0).toString()))
		...
		if (display.count.get(1).intValue() > 0) {
			...
			widgets.add(Widgets
					.createLabel(new Point(display.count.get(1).intValue() < 10 ? startPoint.x + 46 : startPoint.x + 43,
							startPoint.y + 1), Component.translatable(display.count.get(1).toString())));
		}
		...
		if (display.count.get(2).intValue() > 0) {
			...
			widgets.add(Widgets
					.createLabel(new Point(display.count.get(2).intValue() < 10 ? startPoint.x + 76 : startPoint.x + 73,
							startPoint.y + 1), Component.translatable(display.count.get(2).toString())));
		}
		...
		if (display.count.get(3).intValue() > 0) {
			...
			widgets.add(Widgets
					.createLabel(new Point(display.count.get(3).intValue() < 10 ? startPoint.x + 16 : startPoint.x + 14,
							startPoint.y + 31), Component.translatable(display.count.get(3).toString())));
		}
		...
		if (display.count.get(4).intValue() > 0) {
			...
			widgets.add(Widgets
					.createLabel(new Point(display.count.get(4).intValue() < 10 ? startPoint.x + 46 : startPoint.x + 43,
							startPoint.y + 31), Component.translatable(display.count.get(4).toString())));
		}
